---
title: "Persuasion and Influence Map"
title_zh: "說服與影響力導覽"
type: "subdomain-index"
domain: "frameworks-and-mental-models"
lang: "en"
updated: "2026-07-19"
status: "evergreen"
owner: "stanley-lu"
---

# Persuasion and Influence Map

> **中文名稱：** 說服與影響力導覽

Use this map for negotiation, framing, risk reversal, social influence, and word-of-mouth models. Return to [[04-Frameworks-and-Mental-Models/Frameworks and Mental Models Index|Frameworks and Mental Models]].

## Start here

- [[Tactical Empathy - Labels, Silence, and the Accusation Audit]] — understand before attempting to move
- [[Calibrated Questions - How and What Create the Illusion of Control]] — reveal constraints without confrontation
- [[Weapons of Influence - Click-Whirr, Trigger Features, and the Contrast Principle]] — understand automatic influence mechanisms
- [[Risk Reversal - The Guarantee Toolkit]] — reduce the cost of action
- [[The STEPPS Framework - Six Drivers of Word of Mouth]] — diagnose shareability

## Conceptual clusters

### Negotiation and discovery

- [[Black Swans - Find Unknown Unknowns and Hidden Leverage]]
- [[Calibrated Questions - How and What Create the Illusion of Control]]
- [[Guarantee Execution - How, the Rule of Three, and Hidden Players]]
- [[Master No - Protect Autonomy to Start the Real Negotiation]]
- [[Mirroring and Voice - Slow the Negotiation and Keep Them Talking]]
- [[Start With Agreement - Keep the Other Mind Open]]
- [[Tactical Empathy - Labels, Silence, and the Accusation Audit]]

### Framing, risk, and deal mechanics

- [[Bend Their Reality - Loss, Fairness, Deadlines, and Anchors]]
- [[Practical Value - The Psychology of Deals and the Rule of 100]]
- [[Risk Reversal - The Guarantee Toolkit]]
- [[Scarcity and Urgency - Engineering Demand with Supply and Deadlines]]

### Social influence and group behavior

- [[People Like Us Do Things Like This - Culture and Status as Behavior]]
- [[Reciprocation and Commitment - Persuasion's Two Deepest Levers]]
- [[Social Proof, Liking, Authority, and Scarcity - The Four Social Shortcuts]]
- [[The Minority Rule - How Intransigent Preferences Dominate Flexible Majorities]]
- [[Trust and Tension - Move People Without Coercion]]
- [[Weapons of Influence - Click-Whirr, Trigger Features, and the Contrast Principle]]

### Virality and word of mouth

- [[High-Arousal Emotion - When We Care, We Share]]
- [[Public - Observability and Behavioral Residue]]
- [[Social Currency - We Share What Makes Us Look Good]]
- [[The STEPPS Framework - Six Drivers of Word of Mouth]]
- [[Triggers - Top of Mind, Tip of Tongue]]
- [[Valuable Virality - Build a Trojan Horse People Can't Retell Without You]]

## Capacity decision

This folder has a dedicated MOC because negotiation, influence mechanisms, deal framing, and virality answer different reader questions. Keep one physical folder while cross-links remain dense; introduce subfolders only if stewardship or retrieval patterns become stable enough to make placement unambiguous.

## Inventory

```dataview
TABLE WITHOUT ID file.link AS "Entry", type AS "Type", lang AS "Lang", tags AS "Topics", status AS "Status", updated AS "Updated"
FROM "04-Frameworks-and-Mental-Models/Persuasion-and-Influence"
WHERE type != "subdomain-index"
AND contains(list("draft", "reviewed", "evergreen"), status)
SORT file.name ASC
```
