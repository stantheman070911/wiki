...
- Navigation lives in two hand-curated surfaces: a sub-topic folder's `… Map.md`, where the cluster is large enough to have one, and the curated sections of a domain's `… Index.md` (Start here, guided paths). The index tables themselves are generated by Dataview and are never edited by hand. An entry added to a folder that has a map belongs in that map.
...
1. **Archive the source** under `06-Source-Library/<Type>/` from `00-Templates/Source-Template.md`. Keep the raw text, but strip stray `#` characters and extraction debris — Obsidian renders them as false tags.
2. **Distill and write entries** from the matching template in `00-Templates/`, placed per the rules above, linking the source and related entries. Update an existing entry rather than writing a near-duplicate.
3. **Wire the entries into the graph:** add each to its subdomain map where one exists, add the return link from every entry you linked to, and touch a domain index only when the batch changes how that domain should be navigated. The check cannot see an orphan; this step is what keeps the vault navigable.
4. **Delete the inbox copy** (not `Inbox Index.md`) and run `npm run check`; fix anything it flags.
5. **Sweep, commit, and push.** Before committing, confirm nothing else went stale — maps, indexes, `_meta/tags.md` and its Boundaries section, backlinks. The check proves links resolve, not that the knowledge base still reads as one consistent whole.
