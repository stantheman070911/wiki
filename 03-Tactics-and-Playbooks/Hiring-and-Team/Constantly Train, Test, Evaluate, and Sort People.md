---
title: "Constantly Train, Test, Evaluate, and Sort People"
lang: "en"
tags: [topic/hiring, topic/feedback, topic/leadership, topic/management, person/ray-dalio, source/bridgewater-associates]
source:
  type: "book"
  name: "Principles"
  author: "Ray Dalio"
  url: ""
  date_of_source: "2017"
date_added: "2026-07-13"
status: "draft"
---

# Constantly Train, Test, Evaluate, and Sort People

## One-line summary
Both people and organizational design have to keep evolving after hiring — assessing "what someone is actually like," honestly and with evidence rather than gut feel or kindness, is the single most powerful lever a manager has, and it has to be done continuously, not just at the annual review.

## Context
Ninth chapter of Work Principles, the second "people" chapter — where [[Hire Right, Because the Penalties for Hiring Wrong Are Huge]] covers getting people in, this chapter covers what happens to them (and how you learn about them) after they're in.

## Key insights
- **Career paths aren't planned upfront — they emerge from discovered strengths and weaknesses.** Training works best on an apprentice model, like a ski instructor alongside a skier: teach people to fish rather than handing them fish, and allow safe mistakes as part of the process. Internalized, experiential learning beats book learning for this kind of judgment.
- **Provide constant feedback**, not just scheduled reviews — the value of feedback decays quickly with distance from the actual event it's about.
- **Evaluate accurately, not kindly** — and treat that as a false tradeoff: accuracy and kindness are ultimately the same thing, because inaccurate praise sets someone up to fail later. Everyone tends to overestimate their own contribution to shared outcomes.
- **Tough love is the hardest, most valuable love to give.** Most people prefer compliments, but accurate criticism is worth more; real behavior change often only happens after someone genuinely "hits bottom" on a specific weakness, and shielding people from that moment can delay the change they actually need.
- **Don't hide your observations about people.** Build any synthesis from specific observations ("dots") upward — don't start from a vague overall impression. Squeeze the dots for meaning, but don't over-squeeze any single dot (one bad at-bat doesn't define a hitter; use batting-average-style aggregation across many observations, not one incident).
- **Make learning about people open, evolutionary, and iterative**, with clear, impartial metrics. The two most common assessment failure modes are overconfidence in your own read of someone and failing to actually get in sync with them about it. Sync nonhierarchically — subordinates should be assessing managers too, not just the reverse.
- **How someone operates matters more than what they happened to produce** — outcomes can be lucky or unlucky in any single instance. "Pay more attention to the swing than the shot," the way you'd judge a poker player across many hands rather than one night's results.
- **When you and someone are genuinely in sync about their weakness, it's probably real** — and it typically shouldn't take more than about a year to know whether someone is a genuine "click" for their role.
- **Train, guardrail, or remove people — don't try to rehabilitate them indefinitely,** since values and abilities rarely change much. Don't "collect" people in roles that don't suit them out of sentimentality, and be willing to remove underperformers you genuinely care about personally. Be cautious about letting someone step back into an old role after failing forward into a new one.
- **The goal of any transfer is the best community-wide use of the person**, not just resolving the immediate friction — and let people "complete their swing" (finish what they're mid-way through) before moving them, where possible.
- **Don't lower the bar.** If someone can't meet the culture's standard within a reasonable time, they need to leave, regardless of how much effort or goodwill has already been invested.

## Framework / model (if applicable)
**Baseball Cards**, evolving over time from accumulated "dots" (individual observations) — see [[Understand That People Are Wired Very Differently - Baseball Cards and the Team Dimensions Profile]]. This chapter is where the tool is actually operated on an ongoing basis: continuous small inputs, aggregated honestly, rather than a once-a-year snapshot.

## Tactics / how to apply
- Give feedback close in time to the event it's about, rather than batching it for a scheduled review months later.
- When evaluating someone, consciously separate "how they operated" from "what the outcome happened to be" — a good process with a bad outcome and a bad process with a good outcome need different responses.
- Before writing off (or over-praising) someone based on a single incident, ask whether you have enough "dots" to support that conclusion, or whether you're over-squeezing one data point.
- Explicitly invite your own direct reports to assess you, not just the reverse — and treat their input as real data, not a formality.
- If you and someone are both aware of a real weakness and it isn't improving after a fair runway (Dalio's rough benchmark: about a year), make the training/guardrail/remove decision rather than extending the runway indefinitely.

## Notable examples
- **The 301% finding.** When Bridgewater asked employees to estimate their own percentage contribution to shared outcomes, the self-reported totals summed to 301% — cited as hard evidence that self-assessment of contribution is systematically inflated and shouldn't be trusted uncorrected.
- **An employee's blunt "D-" grade email to Dalio himself**, sent after a client meeting and copied to colleagues, is used as the standing illustration of genuine nonhierarchical sync — subordinates assessing leadership as openly as the reverse.

## Related entries
- [[Hire Right, Because the Penalties for Hiring Wrong Are Huge]]
- [[Remember That the WHO Is More Important Than the WHAT]]
- [[Create a Culture Where Mistakes Are Okay But Not Learning From Them Isn't]]
- [[Manage as Someone Operating a Machine to Achieve a Goal]]

## Source reference
Ray Dalio, *Principles*, Part III, Ch. 9 ("Constantly Train, Test, Evaluate, and Sort People"). Extraction notes in [[2026-07-13_Book_RayDalio_Principles]]; raw text archived alongside.
