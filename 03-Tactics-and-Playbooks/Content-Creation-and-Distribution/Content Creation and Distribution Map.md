---
title: Content Creation and Distribution Map
title_zh: 內容創作與分發導覽
lang: en
---

# Content Creation and Distribution Map

> **中文名稱：** 內容創作與分發導覽

Use this map for capturing ideas, developing messages, producing short-form and long-form assets, testing creative, and distributing or launching finished work. Return to [[03-Tactics-and-Playbooks/Tactics and Playbooks Index|Tactics and Playbooks]].

## Start here

- [[The Content Repository - Four Topic Buckets and Tweet-First Capture]] — establish the reusable input system
- [[Clear Over Clever - Write Below Third Grade and Meet the Avatar]] — make the message immediately understandable
- [[Promise, Proof, Path - Engineering the Video Intro]] — structure attention and credibility
- [[Creative Experiments - Opposites, Constraints, A-B Tests, and Context]] — test rather than guess
- [[The Waterfall Distribution Method - One Pillar, a Week of Content]] — distribute one strong asset across channels

## Conceptual clusters

### Capture, planning, and iteration

- [[The Content Repository - Four Topic Buckets and Tweet-First Capture]]
- [[Capture Dont Create - Engineering Proof at Scale]]
- [[Content Planner and the Analyze-Double-Down Loop]]
- [[Creative Experiments - Opposites, Constraints, A-B Tests, and Context]]
- [[Double-Down and the 70-20-10 Rule - Templatizing Viral Outliers]]
- [[Writing as a Discovery Tool - Useful, Simple, and Rewritten]]

### Messaging and copy

- [[Clear Over Clever - Write Below Third Grade and Meet the Avatar]]
- [[Copywriting Fundamentals - AIDA, the Headline, and the Slippery Slope]]
- [[Direct-Response Copywriting]]
- [[Declarative to Sell, Procedural to Deliver]]
- [[The StoryBrand Messaging Campaign - Curiosity, Enlightenment, Commitment]]
- [[短影音文案全公式：標題、開頭、結尾與腳本結構]]

### Video, hooks, and production

- [[Hacking Attention - Topic, Format, Hook and the Content Engine]]
- [[Hyperdopamine Ads - Pattern Interrupt, Curiosity, and Native Ad Anatomy]]
- [[Promise, Proof, Path - Engineering the Video Intro]]
- [[Silent Film Storytelling - Struggle Hook, Rising Action, Timestamp, Pacing]]
- [[The Seven Factors of a Short - Topic to Editing]]
- [[包裝、網紅與 2026 年 TikTok 戰術]]
- [[口播、拍攝與剪輯實戰技巧]]
- [[短影音平台戰術與工具箱]]

### Distribution, launches, and monetization

- [[Creator Monetization Ladder and Content-Concept Formulas]]
- [[Story-Selling - Heros Journey, the Trojan Horse, and Handling Objections]]
- [[The Waterfall Distribution Method - One Pillar, a Week of Content]]
- [[The Whisper-Tease-Shout Launch]]

## Capacity decision

This folder has a dedicated MOC because production mechanics, message design, testing, and distribution are distinct retrieval jobs even though they remain one operating system. Keep the folder flat while these four linked groups remain understandable; reconsider physical subfolders when ownership separates or a group exceeds roughly 25 active pages.

## Inventory

```dataview
TABLE WITHOUT ID file.link AS "Entry", lang AS "Lang", status AS "Status", updated AS "Updated"
FROM "03-Tactics-and-Playbooks/Content-Creation-and-Distribution"
WHERE status
SORT file.name ASC
```
